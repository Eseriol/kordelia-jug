...
public class GeminiClassifier implements Classifier {
    ...
    public GeminiClassifier(ChatClient.Builder chatClientBuilder, AiKillSwitch killSwitch, ActivityLog activityLog) {
    ...
    @Bulkhead(name = "aiProvider")
    @Override
    public TriageResult classify(IncomingMail mail) {
        if (killSwitch.isDisabled()) {
            throw new AiUnavailableException("AI wyłączone przełącznikiem demo (kill switch)");
        }
    ...
    private TriageResult toTriageResult(String rawModelOutput, IncomingMail mail) {
        ...
        return switch (normalized) {
            case String s when s.contains("OFERTA_SZKOLEN") ->
                    new TriageResult(new MailCategory.TrainingOffer(mail.senderAddress()), 0.80, false);
            case String s when s.contains("FAQ") ->
                    new TriageResult(new MailCategory.Faq(mail.subject()), 0.80, false);
            case String s when s.contains("DOKUMENTACJA") ->
                    new TriageResult(new MailCategory.Documentation(guessFileName(mail)), 0.75, false);
            default ->
                    new TriageResult(new MailCategory.Other(truncate(mail.body(), 80)), 0.45, false);
        };
    }
